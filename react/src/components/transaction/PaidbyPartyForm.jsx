...
function PaidbyPartyForm(props) {
  ...
  const handleCreate = async (e) => {
    ...
    const response = await ApiPost(`${BACKEND_API_URL}/transactions/paid_by_party`, payload);
    if (response.status === 202) {
        setError(response.data.message);
    }else {
        console.log('Creating paid_by_party transaction:', payload);
        setError('');
        props.setMethod('');
    }
  }
...
export default PaidbyPartyForm